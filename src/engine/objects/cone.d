...
class Cone : Geometry {
  this(float radius = 0.5f, float height = 1.0f, uint numSegments = 128, float[4] color = [1.0f, 1.0f, 1.0f, 1.0f]){
    ...
    for (uint i = 0; i < numSegments; ++i) {
      ...
      vertices ~= Vertex([positions[0].x, positions[0].y - halfHeight, positions[0].z], faceNormal, [-1.0f, -1.0f, 1.0f], [0.0f, 1.0f]);
      vertices ~= Vertex([positions[1].x, positions[1].y - halfHeight, positions[1].z], faceNormal, [-1.0f, -1.0f, 1.0f], [1.0f, 1.0f]);
...
pure void computeCap(T)(T geometry, float[3] center, float[3] normal, float radius, uint numSegments, float[4] color) nothrow {
  for (uint i = 0; i < numSegments; ++i) {
    ...
    geometry.vertices ~= Vertex(p1, normal, [-1.0f, -1.0f, 1.0f], texCoords[0]);      // V0 relative to vIdx
    geometry.vertices ~= Vertex(p2, normal, [-1.0f, -1.0f, 1.0f], texCoords[1]);      // V1 relative to vIdx
    geometry.vertices ~= Vertex(center, normal, [-1.0f, -1.0f, 1.0f], [0.5f, 0.5f]);  // V2 relative to vIdx
