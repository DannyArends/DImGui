...
void showLightsContent(ref App app, uint font = 0) {
  ...
  if(igBeginTable("Sun_Tbl", 2, ImGuiTableFlags_SizingFixedFit, ImVec2(0,0), 0.0f)) {
      ...
      if(igBeginTable(cstr("Light_Tbl_%d", i), 2, ImGuiTableFlags_SizingFixedFit, ImVec2(0,0), 0.0f)) {
        ...
        if(first < 0) {
          ...
          foreach(c; 0 .. NUM_CASCADES) {
            ...
            infoRow(iconTextStr(format("Cascade %d", c), cast(string)ICON_FA_MOON), "%dx%d  r=%.0f texel=%.4f",
                    app.shadows.slots[first + c].extent.width, app.shadows.slots[first + c].extent.height, r, texel);
          }
        } else {
