...
struct DrawInstance {
  ...
  this(uint[2] d, Matrix m = Matrix.init) { meshdef[0..2] = [cast(int)d[0], cast(int)d[1]]; matrix = m; }
  this(uint[2] d, float[4] c, Matrix m = Matrix.init) { meshdef[0..2] = [cast(int)d[0], cast(int)d[1]]; color = c; matrix = m; }
