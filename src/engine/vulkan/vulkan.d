...
extern(C) @nogc void stop(const(char)* title, const(char)* message) nothrow {
  SDL_Log("[FATAL] %s: %s", title, message);
  if(!SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, title, message, null)) { SDL_Log("[FATAL] message box failed: %s", SDL_GetError()); }
  exit(1);
}
