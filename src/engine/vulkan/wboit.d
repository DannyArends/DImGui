...
import matrix : translate, multiply, scale, rotate;
...
void testWBOIT(ref App app) {
  auto red = new Square([1.0f, 0.0f, 0.0f, 0.6f]);
  red.isOpaque = false;
  red.castShadow = false;
  red.instances = [DrawInstance(translate([8.0f, 3.0f, 2.5f]).rotate([0.0f,90.0f,0.0f]).scale([3.0f,3.0f,3.0f]))];
  app.objects ~= red;

  auto fence = new Square([0.4f, 0.4f, 0.4f, 1.0f]);
  fence.isOpaque = false;
  fence.castShadow = false;
  fence.instances = [DrawInstance(translate([8.25f, 3.0f, 3.0f]).rotate([90.0f,90.0f,0.0f]).scale([4.0f,4.0f,4.0f]))];
  fence.opacity("Metal_Grill_01_opacity");
  app.objects ~= fence;

  auto green = new Square([0.0f, 1.0f, 0.0f, 0.6f]);
  green.isOpaque = false;
  green.castShadow = false;
  green.instances = [DrawInstance(translate([8.5f, 3.0f, 3.5f]).rotate([0.0f,90.0f,0.0f]).scale([3.0f,3.0f,3.0f]))];
  app.objects ~= green;
}
