...
void createFramebuffers(ref App app) {
  ...
  for (size_t i = 0; i < app.swapchain.swapChainImageViews.length; i++) {
    ...
    enforceVK(vkCreateFramebuffer(app.dev, &framebufferInfo, null, &app.swapchain.swapChainFramebuffers[i]));
  }
  SDL_Log("%d Framebuffers created", app.swapchain.swapChainFramebuffers.length);
}
