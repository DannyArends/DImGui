...
import geometry : GeometryInstanceData;
...
void updateVertexBuffer(ref App app) {
  for(size_t j = 0; j < app.geometry.length; j++) {
    ...
    void* data;
    vkMapMemory(app.dev, stagingBufferMemory, 0, size, 0, &data);
    ...
    SDL_Log("Vertex buffer holding %d bytes re-created", size);
  }
}
...
void createVertexBuffers(ref App app) {
  for(size_t j = 0; j < app.geometry.length; j++) {
    ...
    void* data;
    vkMapMemory(app.dev, stagingBufferMemory, 0, bufferSize, 0, &data);
    ...
    vkDestroyBuffer(app.dev, stagingBuffer, null);
    vkFreeMemory(app.dev, stagingBufferMemory, null);
    SDL_Log("Vertex buffer holding %d bytes created", bufferSize);
  }
}
...
void createIndexBuffers(ref App app) {
  for(size_t j = 0; j < app.geometry.length; j++) {
    ...
    vkDestroyBuffer(app.dev, stagingBuffer, null);
    vkFreeMemory(app.dev, stagingBufferMemory, null);
    SDL_Log("Index buffer holding %d bytes created", bufferSize);
  }
}
...
void createInstanceBuffers(ref App app) {
  for(size_t j = 0; j < app.geometry.length; j++) {
    ...
    vkDestroyBuffer(app.dev, stagingBuffer, null);
    vkFreeMemory(app.dev, stagingBufferMemory, null);
    SDL_Log("Instance buffer holding %d bytes created", bufferSize);
  }
}
